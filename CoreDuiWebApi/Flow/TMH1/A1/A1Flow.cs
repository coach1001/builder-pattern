...
using CoreDuiWebApi.Flow.TMH1.A1.Constants;
            ...
            var flow = moduleBuilder
                .WithFlow<A1Model, A1Context>("a1")
                    ...
                    .WithStep(m => m.Data)
                        ...
                        .AddControl(m => m.Grading, ControlType.Boolean, "Show Sieves").PositionConfig("1", "11").End()
                        .AddDecorator("Grading").WithMetadata("textWeight", "bold").PositionConfig("1/3", "12").End()                        
                        .AddArray(m => m.Sieves)
                            .PositionConfig("1/3", "13")
                            .GridConfig("1fr 1fr 1fr 1fr")    
                            .WithReactivity(m => m.Grading == true, ReactivityType.VisibleWhen)
                            .WithReactivity(m => m.Grading == false, ReactivityType.ClearWhen)
                            .WithDefaultValue(SievesConstant.A1Sieves)                            
                            .AddControl(m => m.Size, ControlType.Number, "Sieve Size").PositionConfig("1").InitiallyDisabled().End()
                            .AddControl(m => m.MassRetained, ControlType.Number, "Mass Retained").PositionConfig("2").End()
                            .AddControl(m => m.PercPassing, ControlType.Number, "Percentage Passing").PositionConfig("3").InitiallyDisabled().End()
                            .AddControl(m => m.PercRetained, ControlType.Number, "Percentage Retained").PositionConfig("4").InitiallyDisabled().End()
                        .End()
                        .AddControl(m => m.GradingModulus, ControlType.Number, "Grading Modulus").InitiallyDisabled().PositionConfig("1", "14").End()
